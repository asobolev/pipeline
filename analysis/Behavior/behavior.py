...
def get_behav_units(session):
    session_data = load_session_data(session, load_aeps=False)
    moseq_file = session_data['moseq_file']

    behav_units = []
    for i, (unit, i_rate) in enumerate(session_data['single_units'].items()):
        ...
        perc_shuf_high = np.percentile(coeffs_shuf, 95)
        perc_chun_low  = np.percentile(coeffs_chun, 5)
        #corr           = corr_glm_fit_orig[0]
        corr           = coeffs_chun.mean()
        if perc_shuf_high < perc_chun_low and corr > 0.25:
            behav_units.append(unit)
